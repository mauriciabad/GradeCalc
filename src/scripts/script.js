...
function generateEditSubjectUIHTML(id, subject, popup) {
  ...
  for (const exam in exams) {
    ...
    <input style="grid-row: ${(2 + examCount * 1)}; grid-column: 3;" class="" type="number" name="mark"       value="${(subject.grades ? subject.grades : {})[exam] || ''}" data-exam="${examCount}" placeholder="-" autocomplete="off" min="0" max="10" step="0.01" disabled>`;
