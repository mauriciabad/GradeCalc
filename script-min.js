...
function updateSubjectCardInfo(id) {
  getCard(id).innerHTML = `<button onclick="deleteSubject('${id}')" class="subject-card-remove">
...
function generateEvaluations(id) {
  let evaluationsHTML = `<div class="evaluation-select"${Object.keys(subjects[id].evaluations).length <= 1 ? ' style="display: none;"' : ''}>
...
function clearNewSubjectUI() {
  let html = generateEditSubjectUIHTML('new', completeSubject({
    creator: displayName,
    creationDate: {
...
function updateNecesaryMark(id) {
  ...
  for (const exam in identifiers.exams) {
    switch (identifiers.exams[exam].operator) {
      case '>=':
        subjects[id].necesaryMarks[evaluation][exam] = identifiers.exams[exam].value;
        break;
    }
  }
...
function editUIUpdateGrid(grid, e, popup) {
  ...
  let conditions = ''; // if input is filled
